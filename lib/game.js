...
Game.prototype = {
	...
	_addPlayer: function(id, name, pos) {
		sys.log("Making " + name + " player");

		var self = this;
		var plrObj = {
			id: id,
			pos: pos || self.players.length,
			name: name,
			score: 0,
			lives: 10,
			paddleSize: 100,
		...
		if(typeof pos === "number") this.players[pos] = plrObj;
		else this.players.push(plrObj);
	},
	_addSpectator: function(id, name) {
		sys.log("All slots filled, making " + name + " spectator");
		this.spectators.push({id: id, name: name});
	}
};

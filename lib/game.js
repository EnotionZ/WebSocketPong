...
Game.prototype = {
	...
	startPong: function() {
		...
		self.incX = 2+parseInt(Math.random()*2, 10);
		self.incY = 2+parseInt(Math.random()*2, 10);
	...
	_addSpectator: function(id, name) {
		var pos = this.spectators.length;
		this.spectators.push(new SpectatorController({id: id, pos: pos, name: name}));
	}
};

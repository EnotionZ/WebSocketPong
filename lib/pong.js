...
var Pong = function(opts) { this.port = opts.port; this.init(); };
Pong.prototype = {
	init: function() {
		var self = this;
		...
		self.client.subscribe('/coord', function(info) { self.coordReceived(info); });
	...
	startPong: function() {
	    ...
	    setTimeout(self.dropPowerup(), 10000);
	},
	...
	dropPowerup: function() {
		...
		setInterval(function() {
			...
			console.log("Gumdrop dropped: type=" + type);
			self.initPowerup(type);
		}, 10000);
	},

	initPowerup: function(type){
		var self = this;

		// initialize powerup size, movement speed, location
		self.size = 20,
		self.x =  (self.boardWidth - self.size) / 2,
		self.y =  (self.boardWidth - self.size) / 2,
		self.incX = self.incX || parseInt(Math.random()*24, 10),
		self.incY = self.incY || parseInt(Math.random()*24, 10);
		...
		var pu = self.powerUpData;

		self.client.publish('/gumdrop', pu);
		...
		setInterval(function() {
			...
			if(pu.x < 0) pu.incX = Math.abs(pu.incX);
			if(pu.x > self.boardWidth-self.size) pu.incX = 0; // hide element here

			if(pu.y < 0) pu.incY = Math.abs(pu.incY);
			if(pu.y > self.boardWidth-self.size) pu.incY = 0; // hide 
